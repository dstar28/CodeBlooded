...
class SafeguardApiClient {
  ...
  ///
  /// Sends the device's real GPS coordinates to the backend, which runs
  /// the point-in-zone check against active geofences (Shapely, server
  /// side — see app/routers/geofences.py) and returns one of:
  /// `{"status": "safe", "inside_zone": false}` or
  /// `{"status": "warning"|"danger", "inside_zone": bool, "zone_id":
  /// ..., "zone_name": ..., "risk_level": ...}`.
  Future<ApiResult<Map<String, dynamic>>> checkGeofence({
    required double latitude,
    required double longitude,
  }) {
    return _send(
      () => http.post(
        _uri('/geofences/check'),
        headers: const {'Content-Type': 'application/json'},
        body: jsonEncode({
          'latitude': latitude,
          'longitude': longitude,
