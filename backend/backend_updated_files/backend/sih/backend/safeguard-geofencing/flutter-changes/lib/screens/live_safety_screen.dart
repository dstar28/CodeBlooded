...
/// admin-only concept that must never surface here. [caution] and
/// [restricted] now also reflect a real geofence check
/// (POST /geofences/check) run against the device's real GPS position on
/// every refresh; see [_checkGeofence]. The "Simulate Safety Check (Demo)"
/// control below is left in place as a manual override for
/// demoing/testing when there's no real backend or active zone nearby.
...
class _LiveSafetyScreenState extends State<LiveSafetyScreen> {
  ...
  // Set from the real geofence check (POST /geofences/check) when the
  // status is caution/restricted; null when safe or unknown.
  String? _zoneName;
  ...
  Future<void> _fetchPosition() async {
    ...
    try {
      ...
      unawaited(_checkGeofence(position));
    } catch (_) {
      if (!mounted) return;
      setState(() {
  ...
  /// Sends the real GPS fix to the backend's geofence check and updates
  /// [_safetyStatus] with the result. Never overrides [_accessState] or
  /// [_errorMessage] — if the backend is unreachable this just leaves the
  /// last known safety status in place rather than surfacing a second
  /// error path on top of the location one above.
  Future<void> _checkGeofence(Position position) async {
    final result = await SafeguardApiClient.instance.checkGeofence(
    ...
    final zoneName = result.data?['zone_name'] as String?;
    ...
    setState(() {
      switch (status) {
        ...
        case 'safe':
          ...
          break;
        default:
          // Unrecognized/missing status — leave the current status as is.
          break;
      }
    });
  }
...
class _CurrentSafetyStatusCard extends StatelessWidget {
  const _CurrentSafetyStatusCard({required this.status, this.zoneName});

  final SafetyStatus status;

  /// Name of the geofence zone from the last real backend check, when
  /// status is caution/restricted. Null for safe/unknown or when the
  /// current status came from the demo simulate control.
